<template>
  <v-card style="height: 200px;" color="#4480A6">
    <v-card-title class="text-h4 font-weight-black text-white mt-6 ml-4">订单查询</v-card-title>
...
import {deleteOrderById, getOrderByUserId} from "../../../../network/order";
import {betchDeleteCartById} from "../../../../network/cart";
...
export default {
  ...
  setup() {
    ...
    const init = () => {
      getOrderByUserId(route.query.id, page.value).then(res=>{
    ...
    const toOrderDetail = (id) => {
      router.push({path: "salerManageOrderDetail", query: {id: id}})
    }
